...
//   may be beneficial. */
//**************************************************************************
const int cost_state = 3000;
const int cost_refv = 1;
const int cost_actuator_use = 50;
const int cost_actuator_change = 100;

//********************************************************************
// Set Variable initial Index
//********************************************************************
size_t x_start = 0;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N; ++t) {
      fg[0] += cost_state * CppAD::pow(vars[cte_start + t], 2);
      fg[0] += cost_state * CppAD::pow(vars[epsi_start + t], 2);
      fg[0] += cost_refv * CppAD::pow(vars[v_start + t] - ref_v, 2);
    }

    // Minimize the use of actuators.
    for (int t = 0; t < N - 1; ++t) {
      fg[0] += cost_actuator_use * CppAD::pow(vars[delta_start + t], 2);
      fg[0] += cost_actuator_use * CppAD::pow(vars[a_start + t], 2);
    }

    // Minimize the value gap between sequential actuations.
    for (int t = 0; t < N - 2; ++t) {
      fg[0] += cost_actuator_change * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += cost_actuator_change * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }
    ...
    for (int t = 1; t < N; ++t) {
	  ...
	  // Polynomial Modeled Current Reference y and psi on Desired Path
      ...
      //************************************************************************
      // Predicted next state based on current state + actuations
	  ...
	  // Difference should be constrained to zero
      fg[1 + x_start + t] = x1 - x1_pred;
      fg[1 + y_start + t] = y1 - y1_pred;
      fg[1 + psi_start + t] = psi1 - psi1_pred;
      fg[1 + v_start + t] = v1 - v1_pred;
      fg[1 + cte_start + t] = cte1 - cte1_pred;
      fg[1 + epsi_start + t] = epsi1 - epsi1_pred;
    }
  }
};
